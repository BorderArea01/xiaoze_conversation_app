...
- Real-time audio conversation loop with `fastrtc` for low-latency streaming. Supported backends:
  ...
  - **OpenAI-compatible Realtime** - for providers or self-hosted services that implement the OpenAI Realtime websocket protocol.
  - **OpenAI-compatible Chat + ASR/TTS** - for providers that expose separate transcription, chat completions, and speech endpoints.
  - **Platform Agent + ASR/TTS** - uses local ASR/TTS but sends conversation turns to the platform terminal WebSocket intelligent agent.
...
| `BACKEND_PROVIDER` | Realtime backend to use: `huggingface` (default), `openai`, `openai_compatible`, `openai_compatible_chat`, `platform_agent`, or `gemini`. |
| `MODEL_NAME` | Optional model override for OpenAI Realtime, OpenAI-compatible Realtime, OpenAI-compatible Chat, or Gemini Live. Defaults to `gpt-realtime-2` for OpenAI and `gemini-3.1-flash-live-preview` for Gemini. Hugging Face uses the server's model selection. |
...
| `OPENAI_COMPATIBLE_API_KEY` | API key for `BACKEND_PROVIDER=openai_compatible`. |
| `OPENAI_COMPATIBLE_BASE_URL` | HTTP base URL for an OpenAI-compatible realtime provider, for example `https://provider.example.com/v1`. |
| `OPENAI_COMPATIBLE_WEBSOCKET_BASE_URL` | Websocket base URL for an OpenAI-compatible realtime provider, for example `wss://provider.example.com/v1`. |
| `OPENAI_COMPATIBLE_VOICE` | Optional provider-specific voice name. |
| `OPENAI_COMPATIBLE_TRANSCRIPTION_MODEL` | Optional transcription model value sent in the realtime session config. Defaults to `gpt-4o-transcribe`. |
| `OPENAI_COMPATIBLE_CHAT_BASE_URL` | Optional chat-completions base URL for `BACKEND_PROVIDER=openai_compatible_chat`. Falls back to `OPENAI_COMPATIBLE_BASE_URL`. |
| `OPENAI_COMPATIBLE_ASR_BASE_URL` | Optional ASR base URL for `BACKEND_PROVIDER=openai_compatible_chat`. Falls back to the chat base URL. |
| `OPENAI_COMPATIBLE_TTS_BASE_URL` | Optional TTS base URL for `BACKEND_PROVIDER=openai_compatible_chat`. Falls back to the chat base URL. |
| `OPENAI_COMPATIBLE_TTS_MODEL` | TTS model for `openai_compatible_chat`. Defaults to `tts-1`. |
| `OPENAI_COMPATIBLE_TTS_RESPONSE_FORMAT` | TTS audio format for `openai_compatible_chat`; currently `pcm` or `wav`. Defaults to `pcm`. |
| `OPENAI_COMPATIBLE_TTS_SAMPLE_RATE` | Sample rate for raw `pcm` TTS output. Defaults to `24000`. |
| `OPENAI_COMPATIBLE_ASR_LANGUAGE` | Optional language hint sent to ASR, for example `zh`. |
| `OPENAI_COMPATIBLE_TURN_SILENCE_MS` | End-of-turn silence threshold for the chat bridge. Defaults to `900`. |
| `OPENAI_COMPATIBLE_VAD_THRESHOLD` | Simple RMS voice activity threshold for the chat bridge. Defaults to `0.012`. |

### OpenAI-compatible Realtime providers

Use this mode for domestic or self-hosted vendors that expose the OpenAI Realtime websocket protocol:

```env
BACKEND_PROVIDER=openai_compatible
MODEL_NAME=your-realtime-model
OPENAI_COMPATIBLE_API_KEY=your-provider-key
OPENAI_COMPATIBLE_BASE_URL=https://provider.example.com/v1
OPENAI_COMPATIBLE_WEBSOCKET_BASE_URL=wss://provider.example.com/v1
OPENAI_COMPATIBLE_VOICE=your-provider-voice
```

The realtime-compatible mode is not a plain chat-completions adapter.

For providers that expose separate OpenAI-compatible ASR, chat, and TTS endpoints, use the chat bridge:

```env
BACKEND_PROVIDER=openai_compatible_chat
MODEL_NAME=your-chat-model
OPENAI_COMPATIBLE_API_KEY=your-provider-key
OPENAI_COMPATIBLE_BASE_URL=https://provider.example.com/v1
OPENAI_COMPATIBLE_TRANSCRIPTION_MODEL=your-asr-model
OPENAI_COMPATIBLE_TTS_MODEL=your-tts-model
OPENAI_COMPATIBLE_TTS_RESPONSE_FORMAT=pcm
OPENAI_COMPATIBLE_VOICE=your-tts-voice
OPENAI_COMPATIBLE_ASR_LANGUAGE=zh
```

If your ASR, chat, and TTS services live at different hosts, set `OPENAI_COMPATIBLE_CHAT_BASE_URL`, `OPENAI_COMPATIBLE_ASR_BASE_URL`, and `OPENAI_COMPATIBLE_TTS_BASE_URL` separately.

Use the platform intelligent agent as the conversation core:

```env
BACKEND_PROVIDER=platform_agent
OPENAI_COMPATIBLE_API_KEY=your-asr-tts-provider-key
OPENAI_COMPATIBLE_ASR_BASE_URL=https://provider.example.com/v1
OPENAI_COMPATIBLE_TTS_BASE_URL=https://provider.example.com/v1
OPENAI_COMPATIBLE_TRANSCRIPTION_MODEL=your-asr-model
OPENAI_COMPATIBLE_TTS_MODEL=your-tts-model
OPENAI_COMPATIBLE_TTS_RESPONSE_FORMAT=pcm

REACHY_PLATFORM_TOKEN=your-platform-token
...
On Windows PowerShell, run the official check with UTF-8 mode enabled:
