...
## v0.6.4 - 2026-06-03 23:52 CST

### 音频链路

- 修复机器人音频链路停在“等待机器人音频链路启动”的问题：realtime idle 检查现在会兼容 `movement_manager=None`，不会因为运动管理器不可用而把音频循环打崩。
- realtime 监听状态更新增加判空保护，避免机器人运动模块未初始化时影响语音对话链路。
- 语音页切换音色时会同步写入当前个性的 voice 覆盖配置，避免热更新或重连后又被个性预设音色覆盖。
