...
from xiaoze_conversation_app.utils import ensure_localhost_bypasses_proxy
...
from xiaoze_conversation_app.audio.startup_config import apply_audio_startup_config
...
class LocalStream:
    ...
    def _init_settings_ui_if_needed(self) -> None:
        ...
        class BackendPayload(BaseModel):
            ...
            platform_device_id: Optional[str] = None
            platform_user_id: Optional[str] = None
            platform_session_id: Optional[str] = None
            platform_terminal_ws_url: Optional[str] = None
            platform_telemetry_ws_url: Optional[str] = None
            platform_http_base_url: Optional[str] = None
            platform_device_name: Optional[str] = None
            platform_device_type_name: Optional[str] = None
        ...
        def _status_payload() -> dict[str, object]:
            ...
            can_proceed_with_openai_compatible = has_openai_compatible_key
            can_proceed_with_platform_agent = has_platform_agent_key
            ...
            return {
                ...
                "can_proceed_with_openai_compatible": can_proceed_with_openai_compatible,
                ...
                "platform": {
                    ...
                    "activate_on_start": os.getenv("REACHY_PLATFORM_ACTIVATE_ON_START", "0").strip()
                    in {"1", "true", "True", "yes"},
                    "http_base_url": os.getenv("REACHY_PLATFORM_HTTP_BASE_URL", "http://192.168.2.236:8088"),
                    ...
                    "device_id": os.getenv("REACHY_PLATFORM_DEVICE_ID", "reachy-mini-001"),
                    "device_name": os.getenv("REACHY_PLATFORM_DEVICE_NAME", "Reachy Mini Wireless"),
                    "device_type_name": os.getenv("REACHY_PLATFORM_DEVICE_TYPE_NAME", "小泽机器人"),
                    "user_id": os.getenv("REACHY_PLATFORM_USER_ID", "1991"),
                    "session_id": os.getenv("REACHY_PLATFORM_SESSION_ID", "session-20260519-001"),
                    "activation_code": os.getenv("REACHY_PLATFORM_ACTIVATION_CODE", "zxomP9"),
                },
            }
        ...
        def _ready() -> JSONResponse:
            try:
                mod = sys.modules.get("xiaoze_conversation_app.tools.core_tools")
        ...
        def _set_backend(payload: BackendPayload) -> JSONResponse:
            ...
            platform_token = (payload.platform_token or "").strip()
            ...
            if backend == PLATFORM_AGENT_BACKEND:
                platform_updates = {
                    ...
                    "REACHY_PLATFORM_DEVICE_ID": payload.platform_device_id or "",
                    "REACHY_PLATFORM_USER_ID": payload.platform_user_id or "",
                    "REACHY_PLATFORM_SESSION_ID": payload.platform_session_id or "",
                    "REACHY_PLATFORM_TERMINAL_WS_URL": payload.platform_terminal_ws_url or "",
                    "REACHY_PLATFORM_TELEMETRY_WS_URL": payload.platform_telemetry_ws_url or "",
                    "REACHY_PLATFORM_HTTP_BASE_URL": payload.platform_http_base_url or "",
                    "REACHY_PLATFORM_DEVICE_NAME": payload.platform_device_name or "",
                    "REACHY_PLATFORM_DEVICE_TYPE_NAME": payload.platform_device_type_name or "",
                    "REACHY_PLATFORM_ACTIVATION_CODE": payload.platform_activation_code or "",
                }
                if payload.platform_enabled is not None:
                    platform_updates["REACHY_PLATFORM_ENABLED"] = "1" if payload.platform_enabled else "0"
                if payload.platform_activate_on_start is not None:
                    platform_updates["REACHY_PLATFORM_ACTIVATE_ON_START"] = (
                        "1" if payload.platform_activate_on_start else "0"
                    )
                self._persist_env_values(platform_updates)
            if backend == HF_BACKEND:
