...
from xiaoze_conversation_app.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from xiaoze_conversation_app.config import config  # noqa: F401
from xiaoze_conversation_app.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from xiaoze_conversation_app.tools.background_tool_manager import BackgroundToolManager
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"xiaoze_conversation_app.tools.{tool_name}"
            try:
