...
const OPENAI_COMPATIBLE_BACKEND = "openai_compatible";
const OPENAI_COMPATIBLE_CHAT_BACKEND = "openai_compatible_chat";
const PLATFORM_AGENT_BACKEND = "platform_agent";
...
const BACKEND_META = {
  ...
  [OPENAI_COMPATIBLE_BACKEND]: {
    label: "OpenAI-compatible Realtime",
    formTitle: "Connect compatible provider",
    inputLabel: "OPENAI_COMPATIBLE_API_KEY",
    placeholder: "provider key",
    saveButton: "Save key",
    changeButton: "Change compatible key",
    readyTitle: "Compatible realtime ready",
    readyCopy: "The compatible realtime provider is configured from your local .env.",
    formCopy: "Paste the provider key once. Endpoint URL and model are configured in the app .env.",
    requiredCredentialsCopy: "Set OPENAI_COMPATIBLE_API_KEY plus endpoint/model values in the app .env before switching.",
    note: "OpenAI-compatible Realtime uses OPENAI_COMPATIBLE_API_KEY, OPENAI_COMPATIBLE_BASE_URL, OPENAI_COMPATIBLE_WEBSOCKET_BASE_URL, and MODEL_NAME.",
  },
  [OPENAI_COMPATIBLE_CHAT_BACKEND]: {
    label: "Chat + ASR/TTS",
    formTitle: "Connect ASR/TTS provider",
    inputLabel: "OPENAI_COMPATIBLE_API_KEY",
    placeholder: "provider key",
    saveButton: "Save key",
    changeButton: "Change provider key",
    readyTitle: "Chat + ASR/TTS ready",
    readyCopy: "The compatible chat, ASR, and TTS bridge is configured from your local .env.",
    formCopy: "Paste the provider key once. Model and endpoint URLs are configured in the app .env.",
    requiredCredentialsCopy: "Set OPENAI_COMPATIBLE_API_KEY plus chat, ASR, and TTS model values in the app .env before switching.",
    note: "Chat + ASR/TTS uses MODEL_NAME, OPENAI_COMPATIBLE_TRANSCRIPTION_MODEL, OPENAI_COMPATIBLE_TTS_MODEL, and optional per-service base URLs.",
  },
  [PLATFORM_AGENT_BACKEND]: {
    label: "Platform Agent",
    formTitle: "Connect platform agent",
    inputLabel: "OPENAI_COMPATIBLE_API_KEY",
    placeholder: "ASR/TTS provider key",
    saveButton: "Save platform config",
    changeButton: "Change ASR/TTS key",
    readyTitle: "Platform agent ready",
    readyCopy: "The platform intelligent agent is configured from your local .env.",
    formCopy: "Paste the ASR/TTS provider key once. Platform token and agent ids are configured in the app .env.",
    requiredCredentialsCopy: "Set OPENAI_COMPATIBLE_API_KEY, REACHY_PLATFORM_TOKEN, REACHY_PLATFORM_DEVICE_ID, and REACHY_PLATFORM_USER_ID before switching.",
    note: "Platform Agent uses local ASR/TTS and sends turns to the platform terminal websocket as ASR_SIGNAL.",
  },
  [GEMINI_BACKEND]: {
    label: "Gemini Live",
...
function backendHasCredentials(status, backend) {
  if (
    ...
    || backend === OPENAI_COMPATIBLE_CHAT_BACKEND
    || backend === PLATFORM_AGENT_BACKEND
  ) {
    if (backend === PLATFORM_AGENT_BACKEND && status.has_platform_agent_key !== undefined) {
      return !!status.has_platform_agent_key;
    }
    return !!status.has_openai_compatible_key;
  }
...
function backendCanProceed(status, backend) {
  if (
    ...
    if (backend === PLATFORM_AGENT_BACKEND && status.can_proceed_with_platform_agent !== undefined) {
      return !!status.can_proceed_with_platform_agent;
    }
    return status.can_proceed_with_openai_compatible !== undefined
      ? !!status.can_proceed_with_openai_compatible
...
function formatBackendNote(text) {
  return text
    ...
    .replace("OPENAI_COMPATIBLE_API_KEY", "<code>OPENAI_COMPATIBLE_API_KEY</code>")
    .replace("OPENAI_COMPATIBLE_BASE_URL", "<code>OPENAI_COMPATIBLE_BASE_URL</code>")
    .replace("OPENAI_COMPATIBLE_WEBSOCKET_BASE_URL", "<code>OPENAI_COMPATIBLE_WEBSOCKET_BASE_URL</code>")
    .replace("OPENAI_COMPATIBLE_TRANSCRIPTION_MODEL", "<code>OPENAI_COMPATIBLE_TRANSCRIPTION_MODEL</code>")
    .replace("OPENAI_COMPATIBLE_TTS_MODEL", "<code>OPENAI_COMPATIBLE_TTS_MODEL</code>")
    .replace("MODEL_NAME", "<code>MODEL_NAME</code>")
...
async function saveBackendConfig(
  backend,
  { key = "", hfMode = "", hfHost = "", hfPort = null, platform = {} } = {},
) {
...
async function init() {
  ...
  const platformToken = document.getElementById("platform-token");
  const platformDeviceId = document.getElementById("platform-device-id");
  const platformUserId = document.getElementById("platform-user-id");
  const platformSessionId = document.getElementById("platform-session-id");
  const platformTerminalUrl = document.getElementById("platform-terminal-url");
  const platformTelemetryUrl = document.getElementById("platform-telemetry-url");
  const platformHttpUrl = document.getElementById("platform-http-url");
  const platformDeviceName = document.getElementById("platform-device-name");
  const platformDeviceType = document.getElementById("platform-device-type");
  const platformActivationCode = document.getElementById("platform-activation-code");
  const platformEnabled = document.getElementById("platform-enabled");
  const platformActivateOnStart = document.getElementById("platform-activate-on-start");
  ...
  function populatePlatformFields(status) {
    const platform = status.platform || {};
    platformToken.value = "";
    platformToken.placeholder = platform.token_configured
      ? "Token configured, leave blank to keep current"
      : "Paste platform token";
    platformDeviceId.value = platform.device_id || "reachy-mini-001";
    platformUserId.value = platform.user_id || "1991";
    platformSessionId.value = platform.session_id || "session-20260519-001";
    platformTerminalUrl.value = platform.terminal_ws_url || "";
    platformTelemetryUrl.value = platform.telemetry_ws_url || "";
    platformHttpUrl.value = platform.http_base_url || "";
    platformDeviceName.value = platform.device_name || "Reachy Mini Wireless";
    platformDeviceType.value = platform.device_type_name || "小泽机器人";
    platformActivationCode.value = platform.activation_code || "zxomP9";
    platformEnabled.checked = !!platform.enabled;
    platformActivateOnStart.checked = !!platform.activate_on_start;
    updatePlatformPreview(status);
  }

  function updatePlatformPreview(status = {}) {
    const tokenReady = !!platformToken.value.trim() || !!status.platform?.token_configured;
    const deviceId = platformDeviceId.value.trim() || "reachy-mini-001";
    const url = platformTerminalUrl.value.trim() || `ws://192.168.2.236:8088/websocket/terminal/${deviceId}`;
    setStatusMessage(
      platformPreview,
      `${tokenReady ? "Token ready" : "Token required"} · terminal ${url}`,
      tokenReady ? "" : "warn",
    );
  }
  ...
  function platformPayload() {
    return {
      platform_token: platformToken.value.trim(),
      platform_device_id: platformDeviceId.value.trim(),
      platform_user_id: platformUserId.value.trim(),
      platform_session_id: platformSessionId.value.trim(),
      platform_terminal_ws_url: platformTerminalUrl.value.trim(),
      platform_telemetry_ws_url: platformTelemetryUrl.value.trim(),
      platform_http_base_url: platformHttpUrl.value.trim(),
      platform_device_name: platformDeviceName.value.trim(),
      platform_device_type_name: platformDeviceType.value.trim(),
      platform_activation_code: platformActivationCode.value.trim(),
      platform_enabled: platformEnabled.checked,
      platform_activate_on_start: platformActivateOnStart.checked,
    };
  }
  ...
  function renderCredentialPanels(status) {
    ...
    const usesApiKeyForm = selectedBackend === OPENAI_BACKEND
      || selectedBackend === OPENAI_COMPATIBLE_BACKEND
      || selectedBackend === OPENAI_COMPATIBLE_CHAT_BACKEND
      || selectedBackend === PLATFORM_AGENT_BACKEND
      || selectedBackend === GEMINI_BACKEND;
    const usesHFForm = selectedBackend === HF_BACKEND;
    const usesPlatformForm = selectedBackend === PLATFORM_AGENT_BACKEND;
    ...
    show(platformFields, usesPlatformForm);
    if (usesHFForm) updateHFControls();
    if (usesPlatformForm) updatePlatformPreview(status);
  ...
  const st = (await waitForStatus()) || {
    ...
    can_proceed_with_openai_compatible: false,
    can_proceed_with_platform_agent: false,
  ...
  [
    ...
    platformSessionId,
    platformTerminalUrl,
    platformTelemetryUrl,
    platformHttpUrl,
  ].forEach((el) => {
    el.addEventListener("input", () => updatePlatformPreview(st));
  });
  [platformEnabled, platformActivateOnStart].forEach((el) => {
    el.addEventListener("change", () => updatePlatformPreview(st));
  });
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    if (!key && selectedBackend !== PLATFORM_AGENT_BACKEND) {
    ...
    if (
      selectedBackend === PLATFORM_AGENT_BACKEND
      && !key
      && !(st.has_openai_compatible_key || st.has_platform_agent_key)
    ) {
      setStatusMessage(statusEl, "Please enter an ASR/TTS provider key.", "warn");
      input.classList.add("error");
      return;
    }
    setStatusMessage(
      ...
      selectedBackend === GEMINI_BACKEND
        ...
        : selectedBackend === OPENAI_COMPATIBLE_BACKEND
          || selectedBackend === OPENAI_COMPATIBLE_CHAT_BACKEND
          || selectedBackend === PLATFORM_AGENT_BACKEND
          ? "Saving compatible provider key..."
          : "Validating API key...",
    );
    input.classList.remove("error");
    try {
      if (selectedBackend === OPENAI_BACKEND) {
        ...
        setStatusMessage(statusEl, "Saving compatible provider key...", "ok");
      }
      await saveBackendConfig(selectedBackend, {
        key,
        platform: selectedBackend === PLATFORM_AGENT_BACKEND ? platformPayload() : {},
      });
      setStatusMessage(statusEl, "Saved. Reloading…", "ok");
      window.location.reload();
    } catch (e) {
      ...
      if (selectedBackend === OPENAI_BACKEND && e.message === "invalid_api_key") {
        ...
        setStatusMessage(
          statusEl,
          e.message === "empty_platform_token"
