...
本文按 Reachy Mini 官方 SDK 与 Hugging Face 发布流程整理，面向当前项目 `reachy_mini_conversation_app`。
...
- `ReachyMiniConversationApp` 继承 `ReachyMiniApp`
- `run(self, reachy_mini, stop_event)` 作为 dashboard 启停入口
- `pyproject.toml` 声明 `reachy_mini_apps` entry point
- `src/reachy_mini_conversation_app/static/` 提供配置 UI
...
在 installed applications 里找到 `reachy_mini_conversation_app`，点击运行。运行后点击配置按钮进入 app UI。
...
```text
https://huggingface.co/spaces/<username>/reachy_mini_conversation_app
```
