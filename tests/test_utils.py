...
def test_initialize_camera_and_vision_propagates_local_vision_init_failures() -> None:
    ...
    with (
        patch("xiaoze_conversation_app.utils.CameraWorker") as mock_camera_worker,
        patch("xiaoze_conversation_app.utils.subprocess.run", return_value=MagicMock(returncode=0)),
        patch(
            "xiaoze_conversation_app.vision.local_vision.initialize_vision_processor",
...
def test_initialize_camera_and_vision_raises_when_local_vision_import_crashes() -> None:
    ...
    with (
        patch("xiaoze_conversation_app.utils.CameraWorker") as mock_camera_worker,
        patch("xiaoze_conversation_app.utils.subprocess.run", return_value=MagicMock(returncode=-4)),
    ):
...
def test_initialize_camera_and_vision_raises_when_head_tracker_init_fails() -> None:
    ...
    with (
        patch("xiaoze_conversation_app.utils.CameraWorker") as mock_camera_worker,
        patch(
            "xiaoze_conversation_app.vision.head_tracking.yolo_process.YoloHeadTrackerProcess",
...
def test_initialize_camera_and_vision_uses_mediapipe_head_tracker_in_process() -> None:
    ...
    with (
        patch("xiaoze_conversation_app.utils.CameraWorker") as mock_camera_worker,
        patch(
            "xiaoze_conversation_app.vision.head_tracking.mediapipe.MediapipeHeadTracker",
