...
from xiaoze_conversation_app.vision.local_vision import (
...
def mock_torch() -> Any:
    ...
    with patch("xiaoze_conversation_app.vision.local_vision.torch") as mock:
...
def mock_transformers() -> Any:
    ...
    with (
        patch("xiaoze_conversation_app.vision.local_vision.AutoProcessor") as proc,
        patch("xiaoze_conversation_app.vision.local_vision.AutoModelForImageTextToText") as model,
    ):
...
def test_vision_processor_initialization_failure(mock_torch: Any) -> None:
    ...
    with patch("xiaoze_conversation_app.vision.local_vision.AutoProcessor") as mock_proc:
...
def test_initialize_vision_processor_success(mock_torch: Any, mock_transformers: Any) -> None:
    ...
    with (
        patch("xiaoze_conversation_app.vision.local_vision.snapshot_download") as mock_download,
        patch("xiaoze_conversation_app.vision.local_vision.os.makedirs"),
        patch("xiaoze_conversation_app.vision.local_vision.config") as mock_config,
    ):
...
def test_initialize_vision_processor_processor_failure(mock_torch: Any) -> None:
    ...
    with (
        patch("xiaoze_conversation_app.vision.local_vision.snapshot_download"),
        patch("xiaoze_conversation_app.vision.local_vision.os.makedirs"),
        patch("xiaoze_conversation_app.vision.local_vision.config") as mock_config,
        patch("xiaoze_conversation_app.vision.local_vision.AutoProcessor") as mock_proc,
    ):
        mock_config.LOCAL_VISION_MODEL = "test/model"
        mock_config.HF_HOME = "/tmp/hf_cache"
