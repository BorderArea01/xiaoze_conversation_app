...
def test_openai_compatible_backend_resolves_model_name() -> None:
    """OpenAI-compatible realtime should accept explicit provider model names."""
    assert (
        config_mod._normalize_backend_provider(config_mod.OPENAI_COMPATIBLE_BACKEND, None)
        == config_mod.OPENAI_COMPATIBLE_BACKEND
    )
    assert config_mod._resolve_model_name(config_mod.OPENAI_COMPATIBLE_BACKEND, "qwen-realtime") == "qwen-realtime"
    assert (
        config_mod._normalize_backend_provider(config_mod.OPENAI_COMPATIBLE_CHAT_BACKEND, None)
        == config_mod.OPENAI_COMPATIBLE_CHAT_BACKEND
    )
    assert config_mod._resolve_model_name(config_mod.OPENAI_COMPATIBLE_CHAT_BACKEND, "qwen-plus") == "qwen-plus"
    assert config_mod._normalize_backend_provider(config_mod.PLATFORM_AGENT_BACKEND, None) == config_mod.PLATFORM_AGENT_BACKEND
    assert config_mod._resolve_model_name(config_mod.PLATFORM_AGENT_BACKEND, "qwen-plus") == "qwen-plus"


def test_openai_compatible_voice_can_come_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Provider-specific voices should be available for OpenAI-compatible realtime."""
    monkeypatch.setattr(config_mod.config, "OPENAI_COMPATIBLE_VOICE", "ChineseFemale")

    assert config_mod.get_default_voice_for_backend(config_mod.OPENAI_COMPATIBLE_BACKEND) == "ChineseFemale"
    assert "ChineseFemale" in config_mod.get_available_voices_for_backend(config_mod.OPENAI_COMPATIBLE_BACKEND)
    assert config_mod.get_default_voice_for_backend(config_mod.OPENAI_COMPATIBLE_CHAT_BACKEND) == "ChineseFemale"
    assert config_mod.get_default_voice_for_backend(config_mod.PLATFORM_AGENT_BACKEND) == "ChineseFemale"
