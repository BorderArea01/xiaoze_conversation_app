...
from xiaoze_conversation_app.config import GEMINI_AVAILABLE_VOICES, config
from xiaoze_conversation_app.console import LOCAL_PLAYER_BACKEND, LocalStream
...
def test_local_stream_persist_personality_clears_legacy_startup_env_overrides(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "xiaoze_conversation_app.config.set_custom_profile",
...
def test_local_stream_launch_waits_for_manual_openai_key_without_download(
    ...
    monkeypatch.setattr("xiaoze_conversation_app.console.time.sleep", MagicMock(side_effect=KeyboardInterrupt))
